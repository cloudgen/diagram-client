# GNU Make / Autotools Build Steps for diagram-client

This document explains how to build the **diagram-client** project using the provided defensive build script `./build.sh`.
...
3. **Build with Autotools**
   ...
   **`target/exe/debug/diagram-client`**
...
4. **Run the Program**
   ...
   ```bash
   ./build.sh make && ./target/exe/debug/diagram-client
   ```
...
The binary will still be placed at **`target/exe/debug/diagram-client`**.
...
- The binary name uses the safe version (`github_client` internally for Makefile variables, but the final executable is named `diagram-client`).
