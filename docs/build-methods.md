# Build Methods for diagram-client

The **diagram-client** is a lightweight native GNOME desktop application that embeds a WebKit2 web view as a dedicated wrapper for diagram.app.
...
**`target/exe/debug/diagram-client`** (Basic, GNU Make, Meson)  
or **`target/debug/diagram-client`** (Gradle)
...
```bash
./build.sh clean && ./build.sh install && ./build.sh build && ./target/exe/debug/diagram-client
```
...
| Run the program             | `./target/exe/debug/diagram-client`           |
