# Basic Build Steps for diagram-client

This document provides the simplest way to build and run the **diagram-client** project using the provided `./build.sh` script.
...
```bash
./build.sh clean && ./build.sh build && ./target/exe/debug/diagram-client
```
...
- Places the executable at: **`target/exe/debug/diagram-client`**
...
./target/exe/debug/diagram-client   # Run the app
```

## Notes

- The simple build is ideal for quick development and testing.
- The final binary is always placed in `target/exe/debug/diagram-client`.
