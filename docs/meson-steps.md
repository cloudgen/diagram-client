# Meson + Ninja Build Steps for diagram-client
...
- Compile with Ninja
- Copy the resulting binary to the consistent location: `target/exe/debug/diagram-client`
...
# 3. The binary will be in builddir/diagram-client
# Copy it to the standard location for consistency:
mkdir -p target/exe/debug
cp builddir/diagram-client target/exe/debug/diagram-client 2>/dev/null || true
```
